...
- **`tasks.position` as a fractional index** stored as `text` (base-62 midpoints, not floats — doubles run out of precision after about fifty insertions between the same two neighbours). Moving a card writes one row.
